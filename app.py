...
# ============================================
# FUNCIÓN PARA DÓLAR - MANUAL (ACTUALIZAS TÚ CADA DÍA)
# ============================================
def get_dolar():
    """Obtiene el valor del dólar desde la base de datos (lo actualizas manualmente)"""
    try:
...
def optimizar_imagen(file, max_width=1024, quality=75):
    ...
    try:
        ...
        # Convertir a RGB si es necesario (para eliminar transparencias)
        ...
        # Crear un objeto similar a archivo para subir
        class OptimizedFile:
...
# ============================================
# FUNCIONES DE SUBIDA A STORAGE CON SUPABASE
# ============================================
...
def verificar_bucket():
    ...
    try:
        ...
        if not bucket_existe:
            st.error("❌ El bucket 'imagenes' no existe. Créalo manualmente en Supabase → Storage")
            return False
        return True
    except Exception as e:
        st.error(f"Error verificando bucket: {e}")
        return False
...
def subir_imagen_storage(file, carpeta="imagenes"):
    ...
    try:
        ...
        # Mostrar cuánto se redujo la imagen
        if hasattr(file, 'size') and hasattr(archivo_optimizado, 'size'):
            original = file.size / 1024
            optimizada = archivo_optimizado.size / 1024
            reduccion = (1 - archivo_optimizado.size / file.size) * 100
            st.caption(f"📸 Imagen optimizada: {original:.0f} KB → {optimizada:.0f} KB (reducción del {reduccion:.0f}%)")
...
if st.session_state.get('es_admin', False):
    ...
    if "📰 Noticias" in admin_opt:
        ...
        st.info("💡 IMPORTANTE: El bucket 'imagenes' debe existir en Supabase Storage")
        ...
        if st.button("🔧 Verificar bucket 'imagenes'"):
            if verificar_bucket():
                st.success("✅ Bucket 'imagenes' existe y está listo")
            else:
                st.error("❌ El bucket 'imagenes' no existe. Créalo en Supabase → Storage")
