...
def add_personaje(nombre, descripcion, logros, imagen, fecha):
    try:
        ...
        # Desactivar otros personajes para esta fecha
...
def inicializar_tabla_personajes():
    ...
    try:
        ...
        response = supabase.table("personajes").select("count", count="exact").limit(1).execute()
    except Exception:
        # Crear tabla personajes
        st.warning("La tabla 'personajes' no existe. Ejecuta este SQL en Supabase:")
        st.code("""
        ...
        try:
            ...
            """).execute()
            st.success("✅ Tabla 'personajes' creada exitosamente")
        except Exception as e:
            st.error(f"Error creando tabla personajes: {e}")
...
/* Contenido principal - fondo oscuro para contraste */
.main > div {
...
/* Texto en el área principal - color claro para contraste */
.main * {
...
# ============================================
# URL de una foto real de Santa Teresa del Tuy (puedes reemplazarla)
FONDO_SANTA_TERESA = "https://www.mippci.gob.ve/wp-content/uploads/2023/01/Imagen-de-Santa-Teresa-del-Tuy.jpg"
...
<div style="display: flex; justify-content: center; gap: 15px; flex-wrap: wrap; margin: 15px 0;">
    <a href="https://api.whatsapp.com/send?text=Santa Teresa al Dia - {APP_URL}" target="_blank" style="display: inline-block; padding: 8px 20px; border-radius: 25px; text-decoration: none; font-weight: bold; color: white; background: #25D366;">📱 WhatsApp</a>
    <a href="https://www.facebook.com/sharer/sharer.php?u={APP_URL}" target="_blank" style="display: inline-block; padding: 8px 20px; border-radius: 25px; text-decoration: none; font-weight: bold; color: white; background: #1877F2;">📘 Facebook</a>
    <button onclick="copyToClipboard('{APP_URL}')" style="display: inline-block; padding: 8px 20px; border-radius: 25px; text-decoration: none; font-weight: bold; color: white; background: #3498db; border: none; cursor: pointer;">📋 Copiar</button>
</div>
<script>
...
with menu_tabs[8]:
    ...
    if personaje:
        ...
        st.info("No hay personaje destacado para hoy")
        st.markdown("---")
        st.markdown("### 📜 Personajes Históricos")
        
        personajes_historicos = get_personajes_historicos()
        if not personajes_historicos.empty:
            for _, p in personajes_historicos.head(5).iterrows():
...
if st.session_state.get('es_admin', False):
    ...
    if "📰 Noticias" in admin_opt:
        ...
        with st.form("fcro"):
            ...
            estado = st.selectbox("Estado", ["Miranda", "Carabobo", "Distrito Capital", "Zulia", "Lara"])
            contenido = st.text_area("Contenido")
            if st.form_submit_button("💾 Guardar"):
        ...
        for _, p in get_personajes_historicos().iterrows():
            with st.expander(f"{p['nombre']} - {p['fecha']}"):
                if p.get('imagen_url'):
...
<div class="bronze-footer">
    <div style="position: relative;">
        ...
        <p style="font-size: 1.8em; letter-spacing: 4px; color: #ffd700 !important; font-family: 'Times New Roman', serif; font-weight: bold;">DESARROLLADO POR WILLIAN ALMENAR</p>
        <p style="color: #ffd700 !important; font-family: 'Times New Roman', serif;">Prohibida la reproducción total o parcial</p>
        <p style="color: #ffd700 !important; font-family: 'Times New Roman', serif;">DERECHOS RESERVADOS</p>
        <p style="color: #ffd700 !important; font-family: 'Times New Roman', serif;">Santa Teresa del Tuy, 2026</p>
    </div>
</div>
""", unsafe_allow_html=True)
