...
# ============================================
# BOTONES DE COMPARTIR (CON COPIA FUNCIONAL)
# ============================================
st.markdown(f"""
...
# ============================================
# ENCABEZADO PRINCIPAL (CON FECHA, HORA Y VISITANTES)
# ============================================
ahora = get_fecha_hora_venezuela()
...
# ============================================
# SIDEBAR ADMIN (TODO EL CONTROL ESTÁ AQUÍ)
# ============================================
with st.sidebar:
...
# ============================================
# MENU PRINCIPAL (TABS) - SOLO LECTURA PARA VISITANTES
# ============================================
...
# --- TAB 8: PERSONAJES QUE HICIERON HISTORIA (SOLO ADMIN PUEDE MODIFICAR) ---
with menu_tabs[8]:
    st.title("👥 Personajes que hicieron historia")
    
    # Formulario para CREAR (solo visible para admin)
    if es_admin:
    ...
    # Lista de personajes (visible para todos)
    ...
    if not personajes.empty:
        for _, p in personajes.iterrows():
            with st.expander(f"👤 {p['nombre']} - {p['fecha']}"):
                ...
                # Botones de modificar/eliminar SOLO para admin
                if es_admin:
                    col1, col2 = st.columns(2)
                    with col1:
    ...
    # Formulario para MODIFICAR (solo admin)
    if es_admin and 'edit_personaje_publico' in st.session_state:
