...
# --- NEGOCIOS (MEJORADO) ---
def add_negocio(nombre, categoria, resena, direccion, telefono, horario, ubicacion_texto, google_maps_url, imagenes):
    try:
        ahora = get_fecha_hora_venezuela()
        ...
        data = {
            ...
            "categoria": categoria,
            "resena": resena,
            "imagenes_url": imagenes_urls,  # Lista de URLs
            "direccion": direccion,
            "telefono": telefono,
            "horario": horario,
...
def update_negocio(id_, nombre, categoria, resena, direccion, telefono, horario, ubicacion_texto, google_maps_url, imagenes):
    try:
        imagenes_urls = None
        if imagenes:
...
def add_opinion_negocio(negocio_id, usuario, comentario, calificacion):
    try:
        ...
        data = {
            ...
            "aprobada": True  # Auto-aprobado para simplificar
        }
...
def inicializar_tabla_opiniones_negocios():
    """Crea la tabla de opiniones de negocios si no existe"""
    try:
        supabase.table("opiniones_negocios").select("count", count="exact").limit(1).execute()
    except Exception:
        # Crear la tabla
        supabase.sql("""
            CREATE TABLE IF NOT EXISTS opiniones_negocios (
...
inicializar_tabla_opiniones_negocios()
...
# --- TAB 2: DONDE IR - DONDE COMPRAR (PÚBLICO) ---
with menu_tabs[2]:
    ...
    if not negocios.empty:
        for _, n in negocios.iterrows():
            with st.expander(f"🏪 {n['nombre']} - {n['categoria']}"):
                ...
                if n.get('ubicacion_texto'):
                    st.write(f"📍 **Ubicación:** {n['ubicacion_texto']}")
...
# ============================================
# PANEL ADMIN - CONTENIDO (SOLO VISIBLE PARA ADMIN)
# ============================================
if st.session_state.get('es_admin', False):
    ...
    # --- NEGOCIOS (MEJORADO CON UBICACIÓN, MAPAS, OPINIONES) ---
        ...
        with st.expander("➕ Crear nuevo negocio", expanded=True):
            with st.form("fneg"):
                ...
                resena = st.text_area("Reseña comercial *")
                direccion = st.text_input("Dirección")
                telefono = st.text_input("Teléfono")
                horario = st.text_input("Horario de atención")
                ubicacion_texto = st.text_input("Ubicación (descripción)")
                ...
                if len(imagenes) > 2:
                    ...
                    if nombre and categoria and resena:
                        add_negocio(nombre, categoria, resena, direccion, telefono, horario, ubicacion_texto, google_maps_url, imagenes)
        ...
        for _, n in get_negocios().iterrows():
            with st.expander(f"🏪 {n['nombre']} - {n['categoria']}"):
                ...
                col1, col2 = st.columns(2)
                with col1:
                    if st.button(f"✏️ Modificar Negocio", key=f"edit_neg_{n['id']}"):
                        st.session_state.edit_negocio = n.to_dict()
                        st.rerun()
                with col2:
                    if st.button(f"🗑️ Eliminar Negocio", key=f"del_neg_{n['id']}"):
                        delete_negocio(n['id'])
        ...
        if 'edit_negocio' in st.session_state:
            ...
            with st.form("edit_negocio_form"):
                ...
                nueva_ubicacion_texto = st.text_input("Ubicación", value=n.get('ubicacion_texto', ''))
                nuevo_google_maps = st.text_input("Google Maps URL", value=n.get('google_maps_url', ''))
                ...
                if len(nuevas_imagenes) > 2:
                    ...
                    if update_negocio(n['id'], nuevo_nombre, nueva_categoria, nueva_resena, nueva_direccion, nuevo_telefono, nuevo_horario, nueva_ubicacion_texto, nuevo_google_maps, nuevas_imagenes):
