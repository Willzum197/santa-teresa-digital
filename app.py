...
# ============================================
# 🖼️ IMAGEN DE FONDO (la que quieres usar)
# ============================================
...
def agregar_likes_automaticos():
    try:
        ...
        if response.data:
            ...
            lote = int(match.group(1)) + 1 if match else 1
        else:
            lote = 1
        for i in range(2):
            data = {
...
def extraer_video_id(url_youtube):
    ...
    patterns = [r'(?:youtube\.com\/watch\?v=)([\w-]+)', r'(?:youtu\.be\/)([\w-]+)', r'(?:youtube\.com\/embed\/)([\w-]+)']
...
# ============================================
# FUNCIONES CRUD (versión resumida pero completa)
# ============================================
...
# ============================================
# 🎨 ESTILOS - IMAGEN DE FONDO CORREGIDA (NUNCA DESAPARECE)
# ============================================
st.markdown(f"""
<style>
/* La imagen de fondo se fija con !important para que nunca desaparezca */
.stApp {{
...
/* Fondo de respaldo en caso de que la imagen no cargue */
.stApp::before {{
    content: "";
    position: fixed;
    top: 0;
...
cols2 = st.columns(4)
secciones2 = ["📜 Crónicas", "🎬 Multimedia", "⚠️ Denuncias", "💬 Opiniones"]
for i, sec in enumerate(secciones2):
    with cols2[i]:
        if st.button(sec, use_container_width=True, key=f"tab_{i+4}"):
            st.session_state.selected_tab = i+4

st.markdown("### 📖 Otras Secciones")
cols3 = st.columns(3)
secciones3 = ["👥 Personajes", "⚖️ El Crimen No Paga", "📅 Efemérides Médicas"]
for i, sec in enumerate(secciones3):
    with cols3[i]:
        if st.button(sec, use_container_width=True, key=f"tab_{i+8}"):
            st.session_state.selected_tab = i+8
    ...
    if "📰 Noticias" in admin_opt:
        ...
        with st.expander("➕ CREAR nueva noticia", expanded=True):
            with st.form("fn"):
                ...
                if st.form_submit_button("📤 Publicar Noticia"):
                    if titulo and contenido and add_noticia(titulo, categoria, contenido, imagen):
        ...
        for _, n in get_noticias().iterrows():
            with st.expander(f"📰 {n['titulo']}"):
                col1, col2 = st.columns(2)
                with col1:
                    if st.button(f"✏️ MODIFICAR", key=f"edit_{n['id']}"):
                        st.session_state.edit_noticia = n.to_dict()
                with col2:
                    if st.button(f"🗑️ ELIMINAR", key=f"del_{n['id']}"):
                        delete_noticia(n['id']); st.rerun()
    
    # Las demás secciones del admin son similares...
    # (por brevedad se incluye la estructura, el código completo está en la versión anterior)
